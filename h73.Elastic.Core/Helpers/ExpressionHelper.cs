...
namespace h73.Elastic.Core.Helpers
{
    ...
    {
        public static Expression<Func<T, T2>> PropertyName<T, T2>(this IEnumerable<T> obj,
            Expression<Func<T, T2>> expression)
        {
            return expression;
        }

        public static Expression<Func<T, T2>> MetaPropertyName<T,T2>(this MetaData<T> obj,
            Expression<Func<T, T2>> expression) where T : new()
        {
            return expression;
        }
        ...
        private static string GetMemberName(Expression expression)
        {
            switch (expression)
            {
                ...
                case MethodCallExpression methodCallExpression:
                    ...
                    if (methodCallExpression.Method.Name == "MetaPropertyName")
                    {
                        return string.Join(".", methodCallExpression.Arguments.Select(GetMemberName));
                    }
